...
async function recallFromFacts(
  ...
  // Query decomposition: generate alternative phrasings to bridge semantic gap.
  // The memories-path already does this but facts-path was missing it, causing
  // 59% retrieval misses where the answer exists but phrasing doesn't match.
  const subQueries = await decomposeQuery(query);

  // Run all 4 strategies in parallel with the primary query
  ...
  // Run sub-queries for vector + BM25 to broaden retrieval coverage.
  // Sub-queries are alternative phrasings generated by GPT-4o-mini.
  const subQueryResults: { id: string; rank: number }[] = [];
  for (const subQuery of subQueries.slice(1)) {
    try {
      const subVector = await embed(subQuery);
      const subVec = await factVectorSearch(apiKeyId, agentId, subVector, 50);
      const subBm25 = await factBm25Search(apiKeyId, agentId, subQuery, 30);
      // Add with offset ranks so primary results are preferred in RRF
      const offset = 200;
      for (const r of subVec) {
        subQueryResults.push({ id: r.id, rank: offset + r.rank });
      }
  ...
  if (subQueryResults.length > 0) {
    metrics.push({ name: "sub-queries", count: subQueryResults.length, topScore: 0, durationMs: 0 });
  }
  logStrategyMetrics(metrics);

  // RRF fusion across all strategy results + sub-query expansions
  const rankedLists = [
    vectorResults.results,
    bm25Results.results,
    graphResults.results,
    temporalResults.results,
    ...(subQueryResults.length > 0 ? [subQueryResults] : []),
